...
public class lb_BirdController : MonoBehaviour {
	...
	void Start () {
		...
		GameObject[] groundTargets = GameObject.FindGameObjectsWithTag("BirdLandPlace");
		GameObject[] perchTargets = GameObject.FindGameObjectsWithTag("BirdLandPlace");
		//find the camera
	...
	IEnumerator UpdateTargets(){
		...
		while(true){
			...
			foreach(Collider hit in hits){
				if (hit.tag == "BirdLandPlace" && !birdGroundTargets.Contains (hit.gameObject)){
	...
	void SpawnBird(){
		if (!pause){
			...
			bird.transform.position = GetSpawnPoints();
	...
	{
		int index = Random.Range(0, lstSpawnpoints.Count);
